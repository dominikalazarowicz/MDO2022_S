...
0. Wstęp
Budowanym programem jest prosty program napisany przeze mnie.
Program przeznaczony jest do systemu Linux Ubuntu.
Założonym artefaktem który ma zostać uzyskany jest plik .deb służący do instalacji programu.
Założoną lokalizają w której ma znaleźć się zainstalowany program jest /home/user/hell/

1. Utworzono obraz "carmellino/ubuntus" za pomocą dockerfile'a:

Obraz ten będzie odpowiedzialny za pobranie programu, dockerfile oraz pliku control.

3. Połączono dockera z dockerhubem za pomocą komendy:
sudo docker login -u carmellino

2. Przesłano utworzony obraz na dockerhub za pomocą komendy:
docker push carmellino/ubuntus

3. Utworzono dockerfile'a o treści:
```
FROM ubuntu:latest
RUN apt-get update -y
RUN apt-get install git -y
...
WORKDIR "/"
```
Dockerfile odpowiedzialny będzie za stworzenie środowiska w którym budowany będzie program.
Początkowo środowisko miało być częścią obrazu pobieranego z dockerhub, jednak pojawiały się błędy przy używaniu komendy cmake. Jest to problem którego niestety nie udało mi się rozwiązać.

4. Utworzonego dockerfile dodano do repozytorium git programu

5. Utworzono plik control o treści:
```
Package: hell
Version: 0.2
Maintainer: King Foo
Architecture: all
Description: hellhellhellhell
```
A następnie dodano go do repozytorium
Plik ten jest potrzebny do zbudowania pliku .deb.

6. utworzono nowy pipeline w jenkinsie ze skryptem:
```
pipeline
{
    agent none
    stages
    {
        ...
        stage('Publish')
        {
            ...
            steps
            {
                ...
                sh 'mkdir hell/home/user'
                sh 'mkdir hell/home/user/hell'
                sh 'cp build/main.cpp hell/home/user/hell/hell.cpp'
                sh 'cp naDevOps/control hell/DEBIAN/control'
                sh 'dpkg-deb --build hell'
                archiveArtifacts artifacts: 'hell.deb', fingerprint: true
            }
